...
const LoginPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      toast.error(status === 401 || status === 403 ? 'Check email or password' : error.response?.data?.message || 'Login failed');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="grid min-h-screen lg:grid-cols-[0.9fr_1.1fr]">
        <section className="flex items-center justify-center px-6 py-10 lg:px-12 lg:py-14">
          <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-slate-900/90 p-7 shadow-2xl shadow-black/30 backdrop-blur-xl lg:p-8">
            ...
            <p className="mt-2 text-sm text-slate-400">Sign in to open your FinAIlytics dashboard.</p>
